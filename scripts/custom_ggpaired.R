...
#'@param point.alpha,line.alpha point and line size, respectively.
...
ggpaired <- function(data, cond1, cond2, x = NULL, y = NULL, id = NULL,
                     ...
                     width = 0.5, 
                     point.size = 1.2, line.size = 0.5,
                     point.alpha = 1, line.alpha = 1,
                     line.color = "black",
                     line.type = "solid" ,point.shape = 21, point.fill = "white",
                     ...
                     ...){
    # hack for fixing .select_vec error
    attach(loadNamespace("ggpubr"))
    ...
    .opts <- list(
        ...
        point.alpha = point.alpha, line.alpha = line.alpha, 
        line.color = line.color, point.fill = point.fill,
        line.type = line.type, point.shape = point.shape,
...
ggpaired_core <- function(data, x = NULL, y = NULL, id = NULL,
                          ...
                          point.alpha = 1, line.alpha = 1, 
                          line.color = "black",
                          line.type = "solid", point.shape = 16, point.fill = "white",
                          title = NULL, xlab = "Condition", ylab = "Value",
                          ggtheme = theme_pubr(),
                          ...)
{
    ...
    p <- ggplot(data, create_aes(list(x = x, y = y))) +
        ...
        geom_exec(geom_line, data = data, group = "id",
                  color = line.color, size = line.size, linetype = line.type, alpha = line.alpha,
                  position = position) +
        geom_exec(geom_point, data = data, color = color, fill = point.fill, size = point.size, shape = point.shape, alpha = point.alpha,
                  position = position)
